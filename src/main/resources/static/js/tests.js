...
const ebikeIdFromPage = document.body.getAttribute("data-ebike-id");
...
function truncateNotes(notes, length = 7) {
    if (!notes) return '-';
...
async function loadTests() {
    ...
    let url = "/api/tests/all";
    if (customerId) {
        url = `/api/tests/user/${customerId}`;
    }
    if (ebikeIdFromPage) {
        url = `/api/tests/bike/${ebikeIdFromPage}`
    }
        ...
        if (tests.length === 0) {
            ...
            tbody.innerHTML = tests.map(test => `
                <tr>
                  ...
                  <td>${test.type ?? '-'}</td>
                  <td>${test.technicianId ?? '-'}</td>
                  <td>${test.customerId ?? '-'}</td>
                  <td>${truncateNotes(test.notes)}</td>
<!--                  <td><button type="button" class="btn btn-sm btn-outline-info redirect-test-results" data-id="${test.uuid}">Results</button></td>-->
                ...
                `<td><button type="button" class="btn btn-sm btn-info disabled redirect-test-results" data-id="${test.uuid}">Results</button></td>`}
                    ...
                    <!--                  <td class="report-button-cell" data-uuid="${test.uuid}" data-status="${test.status}">Checking...</td>-->
    ...
    document.querySelectorAll('.details-btn').forEach(button => {
        button.addEventListener('click', async () => {
            ...
            try {
                ...
                document.getElementById("testNotesTextarea").value = test.notes ?? "";
...
document.getElementById("saveNotesBtn").addEventListener("click", async () => {
    const newNotes = document.getElementById("testNotesTextarea").value;

    try {
        const response = await fetch(`/api/tests/${currentTestId}`, {
